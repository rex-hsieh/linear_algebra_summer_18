...
\begin{enumerate}
	...
	\item[Extra Credit 2] Finding a set of vectors whose span has the same dimension as $H$ does not guarantee that the span is the same as $H$! With the aid of \lemref{lem1}, one can conclude this (and the proof is deceptively elegant). For those who stated and proved \lemref{lem1} implicitly (of which there were two, or three), the effort is much appreciated. For those who didn't understand the distinction, please revisit \lemref{lem1} again.
	
	In addition, in the ``logically equivalent'' formation in \thmref{thmbasis}, the terms ``maximal'' and ``minimal'' are used while acknowledging that the sets have a partial ordering. Do not worry about this if you have never done set theory properly before.
